    ...
    */
    ...
    {      
            ...
            if(player->GetTeam() == ALLIANCE)
                    snprintf(message, 250, "[Battle For Halaa]: Alliance has %u resources!", m_ally_gathered);
            else
                    snprintf(message, 250, "[Battle For Halaa]: Horde has %u resources!", m_horde_gathered);
    ...
    bool OutdoorPvPBT::Update(uint32 diff)
    {
            ...
            if(m_FirstLoad == false)
            {
                    if(m_LastWin == ALLIANCE)
                       //sLog->outInfo("[OutdoorPvPBT]: Battle For Halaa has started! Last Winner: Alliance(%u)", ALLIANCE);
                    //else if(m_LastWin == HORDE)
                       //sLog->outString("[OutdoorPvPBT]: Battle For Halaa has started! Last Winner: Horde(%u)", HORDE);
                    //else if(m_LastWin == 0)
                       //sLog->outString("[OutdoorPvPBT]: Battle For Halaa has started! Last Winner: Neutral(0)");
                    m_FirstLoad = true;
            }
            ...
            if(IS_ABLE_TO_SHOW_MESSAGE == true) // This will limit the spam
            {
                    ...
                    for(SessionMap::iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr) // We're searching for all the sessions(Players)
                    {
                            ...
                            if(itr->second->GetPlayer()->GetZoneId() == 3537)
                            {
                                    ...
                                    {
                                            snprintf(msg, 1024, "[Battle For Halaa Defense]: Alliance has %u resources remaining!", m_ally_gathered);
                                    ...
                                    {
                                            snprintf(msg, 1024, "[Battle For Halaa Defense]: Horde has %u resources remaining!", m_horde_gathered);
                                            itr->second->GetPlayer()->MonsterTextEmote(msg, itr->second->GetPlayer()->GetGUID());
                                    }
                                    ...
                                    if(IS_RESOURCE_MESSAGE_A == true)
                                    {
                                            ...
                                            {
                                                    itr->second->GetPlayer()->MonsterTextEmote("[Battle For Halaa Defense]: Alliance has 0 resources! Horde wins!",
                                    ...
                                    else if(IS_RESOURCE_MESSAGE_H == true)
                                    {
                                            ...
                                            {
                                                    itr->second->GetPlayer()->MonsterTextEmote("[Battle For Halaa Defense]: Horde has 0 resources! Alliance wins!",
                                                    itr->second->GetPlayer()->GetGUID());
