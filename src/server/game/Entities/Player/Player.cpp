...
#pragma warning(default:4355)
	...
	vendorentry = -1;
	//This is default entry when you check this number it means it reads original vendor items
...
bool Player::BuyItemFromVendorSlot(uint64 vendorguid, uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
	...
	//Now on this way we will tell client we are buying things from other vendor
	if(vendorentry != -1)
		vItems = sObjectMgr->GetNpcVendorItemList(vendorentry); // else normal vendor
	else
    ...
    if (crItem->maxcount != 0 || vendorentry == -1)
    {
